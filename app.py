"""Streamlit control tower for demand, inventory and supplier risk."""

import pandas as pd
import streamlit as st

st.set_page_config(page_title="Supply Chain Control Tower", layout="wide")
st.title("Intelligent Supply Chain Control Tower")
st.caption("Demand forecasting • Inventory health • Supplier risk • Action priorities")

@st.cache_data
def load_csv(path):
    try:
        return pd.read_csv(path)
    except FileNotFoundError:
        return pd.DataFrame()

control = load_csv("data/control_tower.csv")
forecast = load_csv("data/30_day_xgboost_forecast.csv")
risk = load_csv("data/supplier_risk.csv")

if control.empty:
    st.warning("Run the pipeline first to generate data/control_tower.csv")
else:
    cols = st.columns(4)
    priority = control.get("priority", pd.Series(dtype=str)).astype(str)
    cols[0].metric("SKUs", f"{len(control):,}")
    cols[1].metric("Urgent", int((priority == "URGENT").sum()))
    cols[2].metric("Critical", int((control.get("inventory_status", pd.Series(dtype=str)).astype(str) == "CRITICAL").sum()))
    cols[3].metric("High Supplier Risk", int((control.get("risk_level", pd.Series(dtype=str)).astype(str) == "HIGH").sum()))

    tab1, tab2, tab3 = st.tabs(["Control Tower", "Forecast", "Supplier Risk"])
    with tab1:
        st.subheader("Priority actions")
        st.dataframe(control, use_container_width=True, hide_index=True)
    with tab2:
        st.subheader("30-day demand forecast")
        if forecast.empty:
            st.info("No forecast output found.")
        else:
            forecast["date"] = pd.to_datetime(forecast["date"])
            st.line_chart(forecast.set_index("date")["forecast_demand"])
            st.dataframe(forecast, use_container_width=True, hide_index=True)
    with tab3:
        st.subheader("Supplier risk")
        if risk.empty:
            st.info("No supplier risk output found.")
        else:
            st.dataframe(risk, use_container_width=True, hide_index=True)
